...
fs.readFile('.credentials/credentials.json', 'utf8', (error, content) => {
    if (error) {
        log.logMessage('Error loading youtube client secret file: ' + error);
        return error;
    }

    authorize(JSON.parse(content), (error, result) => {
        log.logMessage(error, result);
        oauthClient.auth = result;
    });
})
